...
Ритм. Чередуй длину предложений, не три подряд одинаковой длины. Два пункта часто лучше трёх в свободном тексте. Это не относится к обязательным трём сценариям прогноза и к нумерованным спискам требований в backlog, там структура задана форматом. Заканчивай абзацы по-разному, не каждый раз афористичной строкой.
...
# ТЕМЫ
Целевое число тем — 4, но это потолок, а не обязанность. Действует правило отбора ниже: лучше меньше тем, чем тема на непроверенном первоисточнике.
...
def fetch_newsapi(api_key: str, max_items: int = 12) -> str:
    try:
        ...
        for a in articles:
            ...
            url_a = a.get("url", "")
            # Фильтр blacklist: домен из url и имя источника (консистентно с fetch_rss)
            try:
                host = (urlparse(url_a).hostname or "").replace("www.", "").lower()
            except Exception:
                host = ""
            if host in BLACKLISTED_DOMAINS:
                continue
            if src and any(bad in src.lower() for bad in BLACKLISTED_SOURCE_NAMES):
                continue
            lines.append(f"• [{src}] {title}\n  {desc}")
        if len(lines) == 1:
            lines.append("(после фильтра blacklist нет записей)")
        return "\n".join(lines)
    except Exception as e:
        return f"=== NEWSAPI ===\nОшибка: {e}"
